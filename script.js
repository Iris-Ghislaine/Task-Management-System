...
// Sample tasks with UUIDs
let tasks = [
  { id: generateUUID(), name: "Finish assignment", dueDate: "2025-09-28", status: "pending" },
  { id: generateUUID(), name: "Doing Roundery", dueDate: "2025-09-29", status: "completed" },
  { id: generateUUID(), name: "Prepare presentation", dueDate: "2025-10-2", status: "pending" },
  { id: generateUUID(), name: "Hiking", dueDate: "2025-10-5", status: "pending" },
  { id: generateUUID(), name: "Meeting with my teammates", dueDate: "2025-09-22", status: "completed" }
];
console.log(tasks);
...
function renderTasks() {
  ...
  filteredTasks.forEach((task) => {
    ...
    // Check if task is overdue (not completed and due date is before today)
    const isOverdue = task.status !== "completed" && task.dueDate !== "No date" && new Date(task.dueDate) < new Date();

    li.innerHTML = `
      <div>
        <p class="font-medium ${task.status === "completed" ? "line-through text-green-600" : isOverdue ? "text-red-600" : ""}">
          ${task.name} ${isOverdue ? "<span class='text-xs text-red-600'>(Overdue)</span>" : ""}
        </p>
        <small class="text-gray-500">Due: ${task.dueDate}</small>
      </div>
      <div class="flex gap-2">
        <button onclick="toggleTask('${task.id}')"
          class="px-2 py-1 ${task.status === "completed" ? "text-yellow-500" : "text-green-500"} text-white rounded">
        ...
        <button onclick="editTask('${task.id}')"
          class="px-2 py-1 text-blue-500 rounded"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button>
...
taskForm.addEventListener("submit", (e) => {
  ...
  if (isNaN(Date.parse(date))) {
    alert("Please enter a valid date in YYYY-MM-DD format (e.g., 2025-10-01).");
    return;
  }
